window.onload= function() {
       ...
       // 4. 定时器部分  难点
        ...
        timer = setInterval(autoplay,2000);   // 每隔2s 调用一次 autoplay
        function autoplay() {
            key++;   // key == 1  先 ++
            // console.log(key, ulLis.length - 1); //  不能超过5
            if(key >= ulLis.length - 1)
            {
               // alert('停下');
                ul.style.left = 0;
                key = 0;  // 因为第6张就是第一张，我们已经播放完毕了， 下一次播放第2张
                // 第2张的索引号是1
            }
        ...
        box.onmouseout = function() {
            timer = setInterval(autoplay,2000);  // 一定这么开
        }
